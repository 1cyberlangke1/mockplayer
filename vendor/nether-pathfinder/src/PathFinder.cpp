...
template<Size size>
std::optional<NodePos> findAir(Context& ctx, const BlockPos& start1x, bool airIfFake) {
    auto start = NodePos{size, start1x};
    auto queue = std::queue<NodePos>{};
    auto visited = std::unordered_set<NodePos>{};
    queue.push(start);
    visited.insert(start);
    if (!isInBounds(ctx.maxHeight, start1x)) return std::nullopt;
    ...
    while (!queue.empty()) {
        ...
        if (isInBounds(ctx.maxHeight, node.absolutePosZero())) {
            // 防御（mockplayer P15）：区块不在缓存时绝不触发生成器——原实现走
            // getChunkNoMutex 用下界生成器生成假 chunk（主世界 y 高处全是固体/
            // 未初始化数据），BFS 找不到空气 → exit(1) 直接杀 JVM 进程（生产
            // 0x20474343 崩溃根因）。改为按 airIfFake 用全空气/全固体兜底：
            // - airIfFake=true（非下界预测）：未加载区块视为空气 → BFS 必能找到起点
            // - airIfFake=false（下界 elytraPredictTerrain）：未加载区块视为固体 →
            //   只在已加载真实区块里找空气，找不到返回无解（Java 侧优雅失败）
            const Chunk& chunk = [&]() -> const Chunk& {
                const ChunkPos chunkPos = blockPos.toChunkPos();
                auto it = ctx.chunkCache.find(chunkPos);
                if (it != ctx.chunkCache.end()) {
                    return *it->second.second;
                }
                return airIfFake ? AIR_CHUNK : SOLID_CHUNK;
            }();
    ...
    // BFS 队列耗尽仍找不到空气 → 无解（原实现 exit(1) 杀 JVM，改为返回 nullopt，
    // pathFind 返回 null → Java 侧 PathCalculationException 优雅失败）
    return std::nullopt;
}

template std::optional<NodePos> findAir<Size::X2>(Context& ctx, const BlockPos& start1x, bool airIfFake);
template std::optional<NodePos> findAir<Size::X4>(Context& ctx, const BlockPos& start1x, bool airIfFake);
